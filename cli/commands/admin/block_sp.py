...
def block_sp(provider_id: str):
    ...
    if provider.blocked:
        raise click.ClickException(f"Storage Provider {utils.int_id_to_f0_str(provider.provider_id)} is already blocked")

    click.confirm(f"Blocking Storage Provider {utils.int_id_to_f0_str(provider.provider_id)}: "
                  f"{utils.json_pretty(provider)}", abort=True)

    tx_hash = SPRegistry().block_provider(provider.provider_id, admin_private_key())
    click.echo(f"Storage Provider {utils.int_id_to_f0_str(provider.provider_id)} blocked: {tx_hash}")
